...
import (
	...
	"github.com/star-table/polaris-backend/service/basic/commonsvc/api"
...
func init() {
	...
	if env == consts.RunEnvGray {
		err := config.LoadNacosConfigAutoConfiguration("commonsvc", env)
		if err != nil {
			panic(err)
		}
	} else {
...
func main() {
	...
	log.Info("redis config: " + strs.ObjectToString(config.GetRedisConfig()))
	...
	if build {
		facadeBuilder := mvc.FacadeBuilder{
			StorageDir: "./../../../facade/commonfacade",

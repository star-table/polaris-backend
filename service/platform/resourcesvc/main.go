...
import (
	...
	"github.com/star-table/polaris-backend/service/platform/resourcesvc/api"
...
func init() {
	...
	if env == consts.RunEnvGray {
		err := config.LoadNacosConfigAutoConfiguration("resource", env)
		if err != nil {
			panic(err)
		}
	} else {
...
func main() {
	...
	if build {
		facadeBuilder := mvc.FacadeBuilder{
			StorageDir: "./../../../facade/resourcefacade",
			Package:    "resourcefacade",
			VoPackage:  "resourcevo",

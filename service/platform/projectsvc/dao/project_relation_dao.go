package dao

import (
	"github.com/star-table/common/library/db/mysql"
	"github.com/star-table/polaris-backend/common/core/consts"
	"github.com/star-table/polaris-backend/common/model/bo"
	"github.com/star-table/polaris-backend/service/platform/projectsvc/po"
	"upper.io/db.v3"
	"upper.io/db.v3/lib/sqlbuilder"
)

func InsertProjectRelation(po po.PpmProProjectRelation, tx ...sqlbuilder.Tx) error {
	var err error = nil
	if tx != nil && len(tx) > 0 {
		err = mysql.TransInsert(tx[0], &po)
	} else {
		err = mysql.Insert(&po)
	}
	if err != nil {
		log.Errorf("ProjectRelation dao Insert err %v", err)
	}
	return nil
}

func InsertProjectRelationBatch(pos []po.PpmProProjectRelation, tx ...sqlbuilder.Tx) error {
	var err error = nil
	isTx := tx != nil && len(tx) > 0
	var batch *sqlbuilder.BatchInserter
	if !isTx {
		conn, err := mysql.GetConnect()
		if err != nil {
			return err
		}
		batch = conn.InsertInto(consts.TableProjectRelation).Batch(len(pos))
	}
	if batch == nil {
		batch = tx[0].InsertInto(consts.TableProjectRelation).Batch(len(pos))
	}
	go func() {
		defer batch.Done()
		for i := range pos {
			batch.Values(pos[i])
		}
	}()
	err = batch.Wait()
	if err != nil {
		log.Errorf("Iteration dao InsertBatch err %v", err)
		return err
	}
	return nil
}

func UpdateProjectRelation(po po.PpmProProjectRelation, tx ...sqlbuilder.Tx) error {
	var err error = nil
	if tx != nil && len(tx) > 0 {
		err = mysql.TransUpdate(tx[0], &po)
	} else {
		err = mysql.Update(&po)
	}
	if err != nil {
		log.Errorf("ProjectRelation dao Update err %v", err)
	}
	return err
}

func UpdateProjectRelationById(id int64, upd mysql.Upd, tx ...sqlbuilder.Tx) (int64, error) {
	return UpdateProjectRelationByCond(db.Cond{
		consts.TcId:       id,
		consts.TcIsDelete: consts.AppIsNoDelete,
	}, upd, tx...)
}

func UpdateProjectRelationByOrg(id int64, orgId int64, upd mysql.Upd, tx ...sqlbuilder.Tx) (int64, error) {
	return UpdateProjectRelationByCond(db.Cond{
		consts.TcId:       id,
		consts.TcOrgId:    orgId,
		consts.TcIsDelete: consts.AppIsNoDelete,
	}, upd, tx...)
}

func UpdateProjectRelationByCond(cond db.Cond, upd mysql.Upd, tx ...sqlbuilder.Tx) (int64, error) {
	var mod int64 = 0
	var err error = nil
	if tx != nil && len(tx) > 0 {
		mod, err = mysql.TransUpdateSmartWithCond(tx[0], consts.TableProjectRelation, cond, upd)
	} else {
		mod, err = mysql.UpdateSmartWithCond(consts.TableProjectRelation, cond, upd)
	}
	if err != nil {
		log.Errorf("ProjectRelation dao Update err %v", err)
	}
	return mod, err
}

func DeleteProjectRelationById(id int64, operatorId int64, tx ...sqlbuilder.Tx) (int64, error) {
	upd := mysql.Upd{
		consts.TcIsDelete: consts.AppIsDeleted,
	}
	if operatorId > 0 {
		upd[consts.TcUpdator] = operatorId
	}
	return UpdateProjectRelationByCond(db.Cond{
		consts.TcId:       id,
		consts.TcIsDelete: consts.AppIsNoDelete,
	}, upd, tx...)
}

func DeleteProjectRelationByOrg(id int64, orgId int64, operatorId int64, tx ...sqlbuilder.Tx) (int64, error) {
	upd := mysql.Upd{
		consts.TcIsDelete: consts.AppIsDeleted,
	}
	if operatorId > 0 {
		upd[consts.TcUpdator] = operatorId
	}
	return UpdateProjectRelationByCond(db.Cond{
		consts.TcId:       id,
		consts.TcOrgId:    orgId,
		consts.TcIsDelete: consts.AppIsNoDelete,
	}, upd, tx...)
}

func SelectProjectRelationById(id int64) (*po.PpmProProjectRelation, error) {
	po := &po.PpmProProjectRelation{}
	err := mysql.SelectById(consts.TableProjectRelation, id, po)
	if err != nil {
		log.Errorf("ProjectRelation dao SelectById err %v", err)
	}
	return po, err
}

func SelectProjectRelationByIdAndOrg(id int64, orgId int64) (*po.PpmProProjectRelation, error) {
	po := &po.PpmProProjectRelation{}
	err := mysql.SelectOneByCond(consts.TableProjectRelation, db.Cond{
		consts.TcId:       id,
		consts.TcOrgId:    orgId,
		consts.TcIsDelete: consts.AppIsNoDelete,
	}, po)
	if err != nil {
		log.Errorf("ProjectRelation dao Select err %v", err)
	}
	return po, err
}

func SelectProjectRelation(cond db.Cond) (*[]po.PpmProProjectRelation, error) {
	pos := &[]po.PpmProProjectRelation{}
	err := mysql.SelectAllByCond(consts.TableProjectRelation, cond, pos)
	if err != nil {
		log.Errorf("ProjectRelation dao SelectList err %v", err)
	}
	return pos, err
}

func SelectOneProjectRelation(cond db.Cond) (*po.PpmProProjectRelation, error) {
	po := &po.PpmProProjectRelation{}
	err := mysql.SelectOneByCond(consts.TableProjectRelation, cond, po)
	if err != nil {
		log.Errorf("ProjectRelation dao Select err %v", err)
	}
	return po, err
}

func SelectProjectRelationByPage(cond db.Cond, pageBo bo.PageBo) (*[]po.PpmProProjectRelation, uint64, error) {
	pos := &[]po.PpmProProjectRelation{}
	total, err := mysql.SelectAllByCondWithPageAndOrder(consts.TableProjectRelation, cond, nil, pageBo.Page, pageBo.Size, pageBo.Order, pos)
	if err != nil {
		log.Errorf("ProjectRelation dao SelectPage err %v", err)
	}
	return pos, total, err
}
